...
| dynamic_match: forall initp currentp nc' ji sync sync' ms ms'
                   (STK_NO: stk_no_anchor (state_stack ms'))
                   (SYNC_NO: synchro_no_anchor sync')
                   (PGM_NO: no_anchor currentp)
               (INTERNAL_SIM: backward_internal_simulation' initp currentp None None nocode nc' AnchorOff AnchorOff (jorder ji) (jrel ji))
               (INTERNAL_MATCH: (jrel ji) (jindex ji) (sync, ms) (sync', ms')),
    match_states initp ji
                 (sync, ms)
                 (Dynamic currentp None sync' (ms', nc') (joptim ji)).
...
Proof.
  ...
  - intros s1 s2 H H0. inv H. inv H0. exists (nb_opt, mkindex refl_type refl_match_states tt refl_order wf_refl).
    ...
    + simpl. auto.
    + apply backward_refl.
    + unfold jrel, jindex. simpl. constructor.
  ...
  - intros i s1 s2 MATCH SAFE. inv MATCH. (* progress *)
    ...
    + right. simpl. simpl in STEP. exists t. destruct s2' as [sync'' ms''].
      exists (Dynamic currentp None sync'' (ms'', nc') (joptim i)). apply exe_step. auto. (* simulation diagram *)
  - intros s2 t s2' DYN_STEP ji s1 MATCH SAFE. inv MATCH. simpl in DYN_STEP. inv DYN_STEP.
    + assert (NO: stk_no_anchor (state_stack ms2) /\ synchro_no_anchor sy2).
      { destruct ms', ms2. apply step_preserve_no_anchor in MIXED; auto.  }
      ...
      * exists (jupdate ji i'). exists (sy1, ms1). split; auto. unfold jupdate. erewrite optim_update.
        destruct NO. constructor; auto. 
      * exists (jupdate ji i'). exists (sy1, ms1). split; auto. right. split; auto.
        ** destruct ji. eapply exec_decreases. simpl. constructor. auto. 
        ** erewrite optim_update. destruct NO. constructor; auto.
    + assert (NO: no_anchor newp).
      { eapply optimize_no_anchor; eauto. }
      ...
      specialize (REFL (S_Call loc, ms')). exploit REFL. constructor. auto.
      ...
      set (newms := bb_ms currentp None nc' AnchorOff (jrel ji) optms).
      ...
      * right. split. apply star_refl. destruct ji. unfold jindex. simpl. apply optim_decreases.
        unfold joptim in H5. simpl in H5. rewrite <- H5. simpl. lia.
      * replace nb_opt with (Nat.pred (joptim ji)) by lia. econstructor; auto.
...
Corollary dynamic_input_correct:
  forall p,
    ...
    backward_simulation (input_sem p)
                        (dynamic_sem p None).
Proof.
  intros p NO. eapply customSmallstep.compose_backward_simulation.
  - apply single_dynamic.
  - eapply input_mixed. 
  - apply dynamic_optim_correct. auto.
Qed.
