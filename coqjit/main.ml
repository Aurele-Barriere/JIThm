...
let main =
  ...
  let cmd_args = [
      ...
      ("-i", Arg.Clear Flags.allow_compile, "Interpreter only, disable generation of native codes");
      ("-n", Arg.Clear Flags.allow_spec, "No Speculation");
      ("-o", Arg.Clear Flags.allow_opts, "No Optimizations at all, middle-end or backend");
    ] in
  let usage () =
